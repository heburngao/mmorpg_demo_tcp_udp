...
Shader "Custom/RimeColor_Face_Normal_Light3" 
{
    Properties
    {
        _MainTex("main tex",2D) = "black"{}
        _MainColor("main color",color) = (1,1,1,1)
        _RimColor("rim front color",Color) = (.1,.1,.1,.1)//边缘颜色
        _RimPowerFront ("rim front power",range(1,10)) = 2//边缘强度
        _RimColor2("rim back color",Color) = (1,1,1,1)//边缘颜色
        _RimPowerBack ("rim back power",range(1,10)) = 2//边缘强度
        ...
        _ChannelMask("ChannelMask",2D) = "white"{}
    }
...
//                float3 viewDir : TEXCOORD2;
                ...
                fixed4 t1 : TEXCOORD2;
                fixed4 t2 : TEXCOORD3;
                fixed2 uvChannelMask: TEXCOORD4;
                LIGHTING_COORDS(5,6)
            };
            sampler2D _MainTex;
            float4 _MainColor;
            ...
            sampler2D _ChannelMask;
            fixed4 _ChannelMask_ST;
...
//                o.uv = v.texcoord;
				...
				o.uvChannelMask = v.texcoord;//TRANSFORM_TEX(v.texcoord,_ChannelMask);
...
//                o.t2 = float4(worldTangent.z,worldBinormal.z,o.worldNormal.z,worldPos.z);
 ...
 //输入uv返回映射之后的uv,xLength为边框的大小   自定议函数需前置
            ...
            {
            	fixed4 channelMaskColor = tex2D(_ChannelMask,IN.uvChannelMask);
                float4 texColor = tex2D(_MainTex,IN.uv)*_MainColor;
				//法线
...
//                c.rgb += pow((1-IN.NdotV.x) ,_RimPowerFront)* _RimColor.rgb ;
//                c.rgb += pow((IN.NdotV.x) ,_RimPowerFront)* _RimColor.rgb ;
//				 float3 cc = pow((1-IN.NdotV.x) ,_RimPowerFront)* _RimColor.rgb ;
				/////
...
//                float atten = LIGHT_ATTENUATION(IN);//自身投影
...
//				float3 worldRefl = reflect(-worldViewDir, IN.worldNormal);
                fixed3 reflCol = texCUBE(_Cubemap,worldRefl).rgb* _ReflAmount;
                 ...
                 //==flow light 流光 
				 float2 noise = tex2D(_Noise,IN.uv + .3f);
                 float2 flowUV = IN.uv + _Time.y * -1;
//				float4 flowUV2 = tex2D(_FlowTex2, IN.uv +flowUV);
//                 flowUV = mapuv(flowUV,1);
  				
                 fixed4 flowTex = tex2D(_FlowTex, TRANSFORM_TEX(flowUV,_FlowTex)+ noise.y*3);//添加水汪汪偏移值
...
//				fixed3 smoothColor	= smoothstep(-1, 1, pow(IN.NdotV,2));
				...
				//yoyo
					   float yoyo =  max(0.1,sin(_CosTime* _NoiseColor * 20)*1);
                 fixed3 flow = flowTex.rgb * saturate( dot(1 - lambert , pow(worldNormal,4))) * 1*yoyo;// * smoothColor;//直接用_NoiseColor会无效
//				fixed3 flow = flowTex.rgb * saturate( dot(1 - IN.NdotV.x , pow(worldNormal,4))) * 1*_NoiseColor * smoothColor;
                 //==
                 //rim=================
                 //yoyo
                float yoyo2 = sin(_SinTime* _RimPowerBack * 3.5)*.5;
                 float3 rim = pow(saturate(IN.NdotV.x) ,_RimPowerFront)* _RimColor.rgb ;
				 float3 rim2 = pow(saturate(1-IN.NdotV.x) ,yoyo2)* _RimColor2.rgb ;
//				float3 rim = pow(saturate(lambert) ,_RimPowerFront)* _RimColor.rgb ;
//				 float3 rim2 = pow(saturate(1-lambert) ,_RimPowerBack)* _RimColor2.rgb ;
				  texColor.rgb += rim+rim2;
                 //==
                fixed4 col;
//                col.rgb = fixed3((ambi + (diff + spec) * atten) * (texColor*.8))+texColor*.8 +reflCol+ flowTex.rgb;//+ rim + reflCol;
				col.rgb = fixed3((ambi + (diff + spec) ) * (texColor*.8))+texColor*.8 +reflCol+ flow * channelMaskColor.b;
//				 col.rgb *= channelMaskColor.b
