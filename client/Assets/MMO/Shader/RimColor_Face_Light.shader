...
// Upgrade NOTE: replaced 'mul(UNITY_MATRIX_MVP,*)' with 'UnityObjectToClipPos(*)'
//人物皮肤带正面光照着色器
Shader "Custom/RimColor_Face_Light" 
{
    Properties
    {
...
//				col.rgb = float3((ambi + (diff + spec) * atten) * texColor) + rim ;//+ reflCol;
//				col.rgb = float3((ambi + (diff + spec) * atten) * texColor) + rim + reflCol;
				float3 nnorm = saturate(dot(rim3 * .8, nor  )) + saturate(dot( rim3 , nor ))*1.2;
				col.rgb = float3((ambi + (diff + spec) * atten) * texColor)+texColor*.8 + nnorm;// + reflCol;
                col.a = 1.0f;
