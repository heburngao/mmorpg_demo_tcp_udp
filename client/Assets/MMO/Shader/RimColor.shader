...
Shader "Custom/RimColor" 
{
    Properties
    {
        ...
        _RimColor("rim front color",Color) = (.11,.11,.11,1)//边缘颜色
        _RimPower ("rim front power",range(1,10)) = 10//边缘强度
        _RimColor2("rim back color",Color) = (.11,.11,.11,1)//边缘颜色
        _RimPower2 ("rim back power",range(1,10)) = 10//边缘强度
        //==
...
//				v.vertex.w = _LightDir;
				float3 V2 = WorldSpaceLightDir(v.vertex.xyzw);
                V2 = mul(unity_WorldToObject,V2);//视方向从世界到模型坐标系的转换
...
//				 float3 cc = pow((1-IN.NdotV.x) ,_RimPower)* _RimColor.rgb ;
				...
				nor.z = sqrt(1- saturate(dot(nor.xy,nor.xy)));
				 ...
				 float3 rim3 = pow(saturate(1-IN.NdotV.y) ,_RimPower2)* _RimColor2.rgb ;
				 texColor.rgb += saturate(dot(rim * .8, nor )) + saturate(dot( rim2 , nor ))*1.2 + saturate(dot( rim3*.5 , nor )) ;
				 /////
