...
Shader "Custom/RimeColor_Face_Normal_Light_NoFaceShadow" 
{
    Properties
    {
...
//        _LightDir1("lightdir1",Range(0.0,255.0)) = .5
//        _LightDir2("lightdir2",Range(0.0,255.0)) = .5
//        _LightDir3("lightdir3",Range(0.0,255.0)) = .5
    }
...
//                float atten = LIGHT_ATTENUATION(IN);//xxxxx 计算光线距离用于投影到自身
                //angle to the light 光线角度
//                float3 diff = _LightColor0.rgb * saturate(dot(normalize(IN.worldNormal), normalize(IN.lightDir)))*1.6;
				float3 diff = _LightColor0.rgb * saturate(dot(normalize(worldNormal), normalize(lightDir)))*1;//lanbert 兰伯特
...
//                col.rgb = fixed3((ambi + (diff + spec) * atten) * (texColor*.8))+texColor*.8 +reflCol;//+ rim + reflCol;
				col.rgb = fixed3((ambi + (diff + spec) ) * (texColor*.8))+texColor*.8 +reflCol;//+ rim + reflCol;
