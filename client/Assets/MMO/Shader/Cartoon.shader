	...
	SubShader
	{
        ...
        Tags{ "LightMode" = "ForwardBase"  "DisableBatching" = "false" }//不指明，会出现明暗显示问题，暗部忽明忽暗
            ...
            //float3 worldPos = mul(unity_ObjectToWorld,i.vertex).xyz;//float3(_LightDir1,_LightDir2,_LightDir3);
                ...
                float3 V2 = WorldSpaceLightDir(i.vertex.xyzw);
                V2 = mul(unity_WorldToObject,V2);//视方向从世界到模型坐标系的转换
                cviewDir.y = saturate(dot(i.worldNormal,normalize(V2)));
                 ...
                 */
                 //====================
                 //rim
                 float rimFace = saturate( dot(normalize(cviewDir.y),i.worldNormal)) < _Outline ? 0 : 1;
                 //tex.rgb += rimFace;
                 //======
				 tex.rgb =  fixed3(floor(tex.rgb*100)/100 * rimFace  * ( ambi  + viewLambert) *1) * _MainColor.xyz +tex.rgb*1 + glass*1;//+ tex.rgb *lambert*2);
