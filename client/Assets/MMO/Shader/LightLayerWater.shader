	...
	SubShader
	{
        ...
        Tags{ "LightMode" = "ForwardBase"  "DisableBatching" = "false" }//不指明，会出现明暗显示问题，暗部忽明忽暗
            ...
            //float3 worldPos = mul(unity_ObjectToWorld,i.vertex).xyz;//float3(_LightDir1,_LightDir2,_LightDir3);
				...
				// sample the texture
                float2 mainFlowUV = i.uv + _Time.y * -.006 ;
                ...
                float3 viewLambert = _LightColor0.rgb * saturate( dot(normalize(i.worldNormal), normalize(cviewDir)));
                 //==flow light 流光 
                 float2 noiseUV = i.uv + _Time.y * .003 ;
                 float2 noiseTex = tex2D(_Noise , TRANSFORM_TEX(noiseUV,_Noise)+.1);//i.uv ,_Noise)+.5);
                 float2 flowUV = i.uv + _Time.y * .5 ;
                 //float2 flowUV = i.uv *  max(0.4, cos(_CosTime* 15));
                 fixed4 flowTex = tex2D(_FlowTex, TRANSFORM_TEX(flowUV.gr,_FlowTex)+ noiseTex.y*2);
                 fixed3 glass = flowTex.rgb * saturate( dot(1 - viewLambert , pow(i.worldNormal,5))) * 1*_NoiseColor;
                 //=================== _GlassCover ======
                 /*
                 ...
                 */
                 ...
                 //tex.rgb += rimFace;
                 //======
				 tex.rgb =  fixed3(tex.rgb   * ( ambi  + viewLambert) *1) * _MainColor.rgb +tex.rgb*1 + glass*1;//+ tex.rgb *lambert*2);
